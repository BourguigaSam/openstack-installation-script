...
function install-magnum-packages-controller() {
	echo "Installing magnum for Controller"
	sleep 2
	DEBIAN_FRONTEND=noninteractive apt-get install magnum-api magnum-conductor python-magnumclient -y
}
...
function install-common-packages() {
	...
	echo "About to configure APT for Stein"
	sleep 3
	apt-get install software-properties-common -y
	ubuntu_version=`lsb_release -sr`
	if [ "$ubuntu_version" == "20.04" ] || [ "$ubuntu_version" == "16.04" ]
	then
		add-apt-repository cloud-archive:stein
	else
		echo "Stein The set up of OpenStack packages is not setted up"
		exit 1;
	fi
...
function install-controller-packages() {
	...
	sleep 2
	install-magnum-packages-controller
...
function install-storage-packages() {
	...
	echo "About to install cinder for Storage Node"
	sleep 2
	apt-get  install lvm2 thin-provisioning-tools -y
...
if [ $# -ne 1 ]
	then
		echo "Correct Syntax: $0 [ allinone | controller | compute | storage ] "
		exit 1;
fi
...
if [ "$1" == "allinone" ]
	...
	then
		echo "installing cinder for storage node.... "
		sleep 5
